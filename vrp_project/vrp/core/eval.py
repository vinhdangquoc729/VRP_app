...
def evaluate(problem: Problem, solution: Solution, return_details=True, hard_tw: bool = False) -> Tuple[float, dict]:
    BIG_PENALTY = 1e14
    BIG_WEIGHT = problem.big_M if problem.big_M else 1e9
    TW_MULTIPLIER = 1e9 if hard_tw else 1.0
    ...
    details = {
        ...
        # vehicle_id -> list of stop schedules, one per route stop (depot + customers + depot)
        "schedules": {},
        # vehicle_id -> {distance_m, duration_s}
        "vehicle_stats": {}
    }
    ...
    for vehicle in problem.vehicles:
        ...
        current_time = 0
        vehicle_schedule = []  # list of per-route stop lists
        ...
        for route in journey:
            ...
            route_stops = []  # stops for this route
            ...
            for i in range(1, len(route.seq)):
                ...
                if node_v:
                    ...
                    if current_time > node_v.end_time:
                        ...
                        details["pc"] += lateness * problem.penalty_weight * TW_MULTIPLIER
                    elif current_time < node_v.start_time:
                        earliness = node_v.start_time - current_time
                        details["pc"] += earliness * problem.penalty_weight * TW_MULTIPLIER

                    stop_entry = {"node_id": v_id, "arrival_time": arrival_time, "departure_time": arrival_time, "orders": []}

                    if hasattr(node_v, 'order_list'):
                        served_customer.add(v_id)
                        service_cursor = current_time  # tracks service start per order
                        for order in node_v.order_list:
                            service_end = service_cursor + order.service_duration
                            stop_entry["orders"].append({
                                "order_id": order.id,
                                "service_start_time": int(service_cursor),
                                "service_end_time": int(service_end)
                            })
                            ...
                            service_cursor += order.service_duration

                        stop_entry["departure_time"] = int(current_time)

                    route_stops.append(stop_entry)
            ...
            vehicle_schedule.append(route_stops)
        ...
        if is_vehicle_used:
            ...
            details["schedules"][vehicle.id] = vehicle_schedule
            details["vehicle_stats"][vehicle.id] = {
                "distance_m": journey_distance,
                "duration_s": journey_duration,
            }
